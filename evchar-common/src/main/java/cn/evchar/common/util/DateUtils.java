...
public class DateUtils {
	...
	public static String DATETIME_FORMATCN = "yyyy年MM月dd�? HH点mm�?";
	...
	/**
	 * Convert date and time to string like "yyyy年MM月dd�? HH点mm�?".
	 */
	public static String formatDateTimeCN(Date d) {
		return new SimpleDateFormat(DATETIME_FORMATCN).format(d);
	}
	/**
	 ...
	 * 
	 * @param formatString
	 *            �?要把目标日期格式化什么样子的格式。例�?,yyyy-MM-dd HH:mm:ss
	 * @param targetDate
	 *            目标日期
	 * @return
	 */
	...
	/**
	 * 比较当前时间与时间date2的天相等 时间格式 2008-11-25 16:30:10 �?:当前时间�?2008-11-25
	 * 16:30:10与传入时�?2008-11-25 15:31:20 相比�?,返回true即相�?
	 * 
	 * @param date1
	 * @param date2
	...
	public static String getChineseWeek(Calendar date) {
		final String dayNames[] = { "星期�?", "星期�?", "星期�?", "星期�?", "星期�?", "星期�?",
				"星期�?" };
	...
	/**
	 * 获取发生日期的结束时�? 根据用户设置的日期天数来判定这这个日期是�?�?(例如 (getHappenMinDate = 2008-10-1) 的话
	 * 那么 (getHappenMaxDate = 2008-11-1) �?)
	 * 
	 * @return
	 */
	...
	public static Date addDay(int num, Date Date) {
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(Date);
		calendar.add(Calendar.DATE, num);// 把日期往后增�? num �?.整数�?后推,负数�?前移�?
		return calendar.getTime(); // 这个时间就是日期�?后推�?天的结果
	}
	...
	/**
	 * 计算两端时间的小时差,精确到分，如1小时40分的返回值为1.67
	 *
	 * @param begin
	 * @param end
	 * @return
	 */
	public static double getHours(Date begin, Date end) {
		Calendar c1 = Calendar.getInstance();
		c1.setTime(begin);
		Calendar c2 = Calendar.getInstance();
		c2.setTime(end);
		...
		Long hour = millisecond / 1000 / 60 / 60;
		Long minute = (millisecond / 1000 / 60) % 60;
		Double res=hour.doubleValue()+(minute.doubleValue()/60);
	 ...
	 * @throws ParseException
	 * 
	 *             �?: DateUtils.formatDate("yyyy-MM-dd HH",new Date())
	 *             "yyyy-MM-dd HH:00:00"
	 */
